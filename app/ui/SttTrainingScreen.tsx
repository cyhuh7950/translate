 ...
 * 한 번의 발화를 통째로 모았다가 WAV 로 감싸 multipart 로 보낸다.
 */
...
export function SttTrainingScreen({
  ...
  async function stopRecording(): Promise<string | null> {
    ...
    try {
      return await capture.stopWithFile();
    } catch (err) {
      setError(errorText(err));
      return null;
    }
  ...
  async function onReadStop() {
    ...
    if (audio === null || prompt === null || prompt.done) return;
    const client = makeClient();
    if (!client || !user) return;
    setBusy(true);
    setError('');
    try {
      ...
      setStatus(prev => (prev ? { ...prev, read: result.progress } : prev));
  ...
  async function onVerifyStop() {
    ...
    try {
      ...
      setVerifyResult({ sampleId: result.sample_id, text: result.recognized_text });
    } catch (err) {
      setError(errorText(err));
    } finally {
      setBusy(false);
  ...
  async function onVerdict(correct: boolean) {
    ...
    try {
      ...
      setStatus(prev => (prev ? { ...prev, verify: result.progress } : prev));
      setVerifyResult(null);
      setShowCorrection(false);
      setCorrectedText('');
    } catch (err) {
