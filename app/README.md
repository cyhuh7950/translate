...
**아직 없는 것**: 온디바이스 ONNX, 네비게이션 라이브러리, **설정 저장**
(고른 값은 앱을 껐다 켜면 서버 기본값으로 돌아간다 — §10).
...
| 입력 방식 | `client.input_modes` | 기본값은 `client.default_input_mode`. **화면 전용이라 서버로 나가지 않는다** (아래 한계) |
| 엔진 | `engines[]` | `kind` 별로 하나씩 — `stt`/`tts` 를 코드가 알지 못하고 응답의 `kind` 를 훑는다. 필드 이름도 `<kind>_engine` 으로 만들어진다 |
| LLM 프로바이더 / 모델 | `llm.providers[]` | 프로바이더가 없는 서버면 두 항목이 다 없다. 모델은 **목록이 응답에 없어서**(프로바이더가 주는 것은 `default_model` 하나뿐) 자유 입력이고, 비워 두면 무엇이 쓰이는지를 자리표시로 알려준다 |
...
- **저장하지 않는다.** 앱을 껐다 켜면 서버 기본값으로 돌아간다. RN 에서 설정을 저장하려면
  `AsyncStorage`(네이티브 의존성)가 필요한데, 이번 변경을 JS 로만 두기로 해서 넣지 않았다.
  **후속 과제다** — 넣을 때는 §1 의 규칙대로 의존성 하나만 따로 추가한다
- **입력 방식은 아직 화면 전용이다.** 실시간 화면은 핸즈프리(VAD) 한 가지로만 돌고
  이 값을 보지 않는다. 서버로도 나가지 않는다(웹에서도 transient 다). 화면에는 남겨 뒀다 —
  `client.input_modes` 가 무엇을 알려주는지 기기에서 보이는 편이 낫기 때문이다
