 ...
 * 서버 `MESSAGE_TO_APP.md` 2026-08-31 확정 계약에 맞춘다.
 */
...
export type SttTrainingNextPrompt =
  | { done: true; progress: SttTrainingProgress }
  | {
      done: false;
      prompt: { id: string; text: string; lang: string };
      progress: SttTrainingProgress;
    };

/**
 * `POST /v1/users/{id}/stt_training/read_sample` 요청.
 *
 * 서버가 요구하는 multipart WAV 본문이다.
 */
...
/** `POST /v1/users/{id}/stt_training/verify` 요청. */
