...
## 결론 — React Native 로 정했다 (2026-08-10)

아래 비교를 거쳐 **React Native** 로 결정했다. 근거가 온디바이스 ONNX 한 축에 크게 쏠려
있으므로, §7 의 실험에서 ONNX 로드가 막히면 네이티브로 다시 볼 수 있다는 것을 함께 적어둔다.
그 외의 축은 어느 쪽을 골라도 큰 손해가 없다(§4).
...
**출발선은 이미 놓여 있다.** `app/` 에 RN 0.86.2 프로젝트와 검증된 `src/api/` 계층이 있고,
2번(WS 왕복)은 **Node 로 이미 통과했다**(스모크 29개). 실기기에서 남은 것은 1번과 3번,
그리고 그 앞에 "앱이 서버에 닿는가"를 눈으로 보는 `App.tsx` 연결 확인 화면이다.
자세한 절차는 `app/README.md`.
